class Location {
  ...
  init() {
    ...
    if (this.currLocItems.length != 0)
      this.object.innerText = "You see " + this.currLocItems[0][1].form;
    ...
    this.commandLine.oninput = () => {
      this.caret.style.left = this.commandLine.value.length * 13.3 + 163 + "px";
      console.log("Backspace before " + this.backspace);
      if (this.backspace == 0)
        if (this.caps == 1) {
          var letter = this.commandLine.value[
            this.commandLine.value.length - 1
          ].toUpperCase();
          var arr = [];
  ...
  render(currItem) {
    ...
    if (currItem != 0)
      this.equipment.innerText = "You are carrying " + currItem.form;
  ...
  useItem(currItem, usedItem) {
    ...
    if (usedItem != currItem.name || currItem == 0) {
      ...
      this.dependencies[this.nameToID[usedItem]].coords != this.locNumber
    ) {
      this.labelStatement("Nothing happened");
      currItem = currItem;
      console.log("item not used");
      ...
      if (this.dependencies[this.nameToID[usedItem]].flag == 0) {
        ...
        if (usedItem == "SHEEP") {
          ...
          this.currLocItems[this.currLocItems.length - 1].push(
            this.dependencies[this.nameToID[usedItem]].newItemId,
            this.items[this.dependencies[this.nameToID[usedItem]].newItemId]
          );
          console.log(this.currLocItems);
          ...
          currItem = 0;
          this.labelStatement(
            this.dependencies[this.nameToID[usedItem]].statement
          );
          this.changeText("eq", currItem);
          this.changeText("obj", this.currLocItems);
          console.log(this.OK);
        ...
        this.dependencies[this.nameToID[usedItem]].statement.includes(
          "(timeout)"
        )
      ) {
        currItem = this.items[
  ...
  dropItem(currItem, dropped) {
    ...
    else if (this.itemsQuantity >= 3)
      this.labelStatement("You can't store any more here");
    else if (dropped != currItem.name)
      this.labelStatement("You are not carrying it");
    else {
  ...
  takeItem(currItem, newItem) {
    ...
    for (i; i < this.currLocItems.length + 1; i++)
      if (i == this.currLocItems.length) {
        ...
        return currItem;
      } else if (newItem == this.currLocItems[i][1].name) break;
    if (currItem != 0) this.labelStatement("You are carrying something");
    else if (!this.items[this.nameToID[newItem]].flag)
      this.labelStatement("You can't carry it");
    else {
      this.currLocItems.splice(i, 1);
  ...
  changeText(id, object) {
    ...
    if (id == "obj") {
      ...
      if (object != 0) string = object.form;
      if (string != "")
  ...
  labelTimeout(currItem, usedItem) {
    var sStatement = this.dependencies[this.nameToID[usedItem]].statement.split(
      "(timeout)"
    );
    ...
    var x = setInterval(() => {
      ...
      if (i == sStatement.length + 1) {
        ...
        this.caret.style.left =
          this.commandLine.value.length * 13.3 + 163 + "px";
        this.cmdLabel.appendChild(this.commandLine);
        this.cmdLabel.appendChild(this.caret);
        this.commandLine.focus();
        this.changeText("eq", currItem);
  ...
  labelDragon(currItem, usedItem) {
    ...
    var x = setInterval(() => {
      ...
      if (i == sStatement.length) {
        this.image.src =
          "img/locations/" + this.dependencies[this.nameToID[usedItem]].graphic;
      }
      if (i == sStatement.length + 1) {
        clearInterval(x);
        this.cmdLabel.innerText = "What now? ";
